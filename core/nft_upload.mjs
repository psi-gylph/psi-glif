 ...
 * 
 * Kullanım:
 * node nft_upload.js --upload <'/Users/banudikici/psi_lab/ψ_DB/ψ-MemoryTrace_Loopback_INIT01.png'> --apiKey <YOUR_LIGHTHOUSE_API_KEY>
 * node nft_upload.js --uploadEncrypted <'/Users/banudikici/psi_lab/ψ_DB/ψ-MemoryTrace_Loopback_INIT01.png'> --apiKey <YOUR_LIGHTHOUSE_API_KEY> --accessKey <erişim_anahtarı>
 * node nft_upload.js --status <cid>
 * node nft_upload.js --getUploads --apiKey <YOUR_LIGHTHOUSE_API_KEY>
 */
import { uploadFile } from './nft_upload.mjs';

const API_KEY = process.env.LIGHTHOUSE_API_KEY;
const FILE_PATH = '/Users/banudikici/psi_lab/ψ_DB/ψ-MemoryTrace_Loopback_INIT01.png';

async function run() {
  const result = await uploadFile(FILE_PATH, API_KEY);
  console.log(result);
}
...
const ψ_GLIF_INTEGRATION = {
  ...
  createGlifEchoSignature: (filePath) => {
    ...
    const hashBuffer = require('crypto').createHash('sha256').update(fileContent).digest();
