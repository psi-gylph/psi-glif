...
*   **Dokümantasyonun Dağınıklığı**: Proje bilgileri, Python dosyaları içinde, ayrı metin bloklarında ve hatta farklı dosyalarda (örn. the originating research note içinde) dağınık haldedir. Bu durum, yeni geliştiricilerin veya kullanıcıların sistemi anlamasını zorlaştırır.
